...
def fit_data(train_data):
    ...
    return artist_median, user_median, global_median

def predict(artist, user, artist_median, user_median, global_median, weight): 
    if artist in artist_median:
        popularity = float(artist_median[artist])/global_median
    else:
        popularity = 1
    if user in user_median:
        return user_median[user]+(popularity*weight)
    else:
...
def validate(val_data, artist_median, user_median, global_median, weight):
    ...
    for user, user_data in val_data.iteritems():
        for artist, true_plays in user_data.iteritems():
            pred_plays = predict(artist, user, artist_median, user_median, global_median, weight)
    ...
    with open(test_file, 'r') as test_fh:
        ...
        with open(soln_file, 'w') as soln_fh:
            ...
            for row in test_csv:
                ...
                if user in user_median:
                    pred = predict(artist, user, artist_median, user_median, global_median, weight)
...
print "Data loaded"
artist_median, user_median, global_median = fit_data(train_data)
print "Fitting data"
